...
def _record_habit_event_lightweight(
    ...
    with contextlib.suppress(Exception):
        ...
        # Path B signal fire accumulation for model telemetry
            ...
            with contextlib.suppress(Exception):
                ...
                for _profile in store.profiles.values():
                    if _profile.confidence > 0 and sum(signal_fires.values()) > 0:
                        ...
                        break
                save_profiles(store)
                signal_fires = {}  # Reset after application
...
def _resolve_verbosity(cp_config, habit_active: bool) -> str:
...
def _should_emit(
    ...
    """
    verbosity = _resolve_verbosity(cp_config, habit_active)

    if verbosity == "full":
        return True

    # Force-emit rules (bypass verbosity)
    if _should_force_emit(report, prev_report):
        return True
...
def _should_force_emit(report: dict, prev_report: dict | None) -> bool:
...
def _extract_habit_signals(session_data: dict) -> tuple[bool, float, float, int]:
...
def _arbitrate_output(
    ...
    habit_active, habit_score, context_pressure, consecutive_failures = (
        _extract_habit_signals(session_data)
    )
    ...
    if getattr(cp_config, "hook_dispositions_enabled", True):
        dispositions = _inject_dispositions(
            session_data,
    ...
    if not _should_emit(cp_config, session_data, habit_active, report, prev_report):
...
def _run_controlplane(input_data: dict, config, cp_config, cwd: str, start: float) -> None:
    ...
    # Message arbitration: apply verbosity gating + disposition injection
    session_data = session.behavior_compass if session else {}
    if not isinstance(session_data, dict):
        session_data = {}
    report = _arbitrate_output(report if report else {}, cp_config, session_data)
