...
class DispositionEnforcer:
    ...
    def evaluate(self, event: SupervisionEvent) -> str | None:
        """Evaluate enforcement rules for the current event.

        Returns a disposition nudge message string if a rule fires, else None.
        """
        ...
        if last_fired:
            ...
            if (
                last_fired == "edit_without_lint"
                and not is_lint
                or last_fired == "cadence_check"
                and tool != "controlplane_run"
            ):
                self._mark_ignore(last_fired)
            elif last_fired == "bash_no_prediction" and not self.config.inquiry.prediction_tracking:
                # If they didn't enable it, they ignored the suggestion
                self._mark_ignore(last_fired)

        disposition = None

        # Rule 1: NUDGE_EDIT_WITHOUT_LINT
        disposition = disposition or self._check_edit_without_lint(event)

        # Rule 2: NUDGE_BASH_WITHOUT_PREDICTION
        disposition = disposition or self._check_bash_without_prediction(event)

        # Rule 3: NUDGE_CONTROLPLANE_CADENCE
        disposition = disposition or self._check_controlplane_cadence(event)
    ...
    def _check_edit_without_lint(self, event: SupervisionEvent) -> str | None:
        ...
        if self.state["flags"].get("needs_lint") and not is_edit and self._can_fire("edit_without_lint"):
