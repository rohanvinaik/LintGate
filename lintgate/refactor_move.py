...
from lintgate.refactor_move_rewrite import generate_shim
...
def _compute_rewrites(
    ...
    for ref in refs:
        if ref.kind == "import" or ref.kind == "from_import" or ref.kind == "string_ref":
...
def _str_to_cst_module(dotted: str) -> Any:
    ...
    # Build right-to-left: foo.bar.baz → Attribute(Attribute(Name(foo), bar), baz)
...
# Extract method moved to lintgate.refactor_extract
from lintgate.refactor_extract import (  # noqa: F401  # re-export
    ExtractResult,
    extract_method,
)
