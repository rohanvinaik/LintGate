...
if TYPE_CHECKING:
    from lintgate.types import LintIssue, Prescription
...
def aggregate(evidence: list[LensEvidence]) -> list[ConvergenceResult]:
    ...
    for target, items in by_target.items():
        ...
        supporting_lenses = sorted(set(e.lens for e in support), key=lambda l: l.value)
        opposing_lenses = sorted(set(e.lens for e in oppose), key=lambda l: l.value)
