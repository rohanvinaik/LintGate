    ...
    for prop in properties.properties:
        if prop.kind == PropertyKind.BOUNDED and prop.bound_spec:
            if prop.bound_spec.lower is not None and prop.bound_spec.upper is not None:
                decorators.append(f"@icontract.ensure(lambda result: {prop.bound_spec.lower} <= result <= {prop.bound_spec.upper})")
            elif prop.bound_spec.lower is not None:
                decorators.append(f"@icontract.ensure(lambda result: result >= {prop.bound_spec.lower})")
            elif prop.bound_spec.upper is not None:
                decorators.append(f"@icontract.ensure(lambda result: result <= {prop.bound_spec.upper})")
