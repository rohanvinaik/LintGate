...
if TYPE_CHECKING:
    from .backends import WitnessRecord
...
def _recognize_group_aggregation(spec: PrescriptiveSpec) -> PatternMatch | None:
    ...
    has_group_signal = _has_invariant_signal(
        spec, {"group", "collect", "bucket", "partition", "categorize"}
    )
...
def synthesize_body(
    ...
    for _, recognizer, generator in _RECOGNIZERS:
        ...
        if witnesses and project_root:
            witness_ok = _validate_against_witnesses(spec, body, witnesses, project_root)
            if not witness_ok:
                return SynthesisResult(
                    success=False,
                    body=body,
