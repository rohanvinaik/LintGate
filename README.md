...
**The automated senior engineer you can't afford to hire — for the vibe-coder who doesn't need one until they do.**
...
- [Tiered Lint Selection](#tiered-lint-selection)
- [The Linter Registry](#the-linter-registry)
- [Anti-Drift Systems](#anti-drift-systems)
- [Context File Management](#context-file-management)
- [Theory Extraction](#theory-extraction)
- [ControlPlane: The Supervision Mesh](#controlplane-the-supervision-mesh)
...
- [MCP Server & Tool Interface](#mcp-server--tool-interface)
...
- Install LintGate and `ruff`.
- Wire the `lintgate` command into Claude Code `PostToolUse`.
- Add optional tools (`mypy`, `radon`, `bandit`, `vulture`) for deeper coverage.
- Use MCP tools for on-demand audits, theory extraction, and diagnostics.
- Enable `controlplane.enabled: true` in `.claude/lintgate.yaml` for the full supervision mesh.
...
| **Enforceable Rules** | Concrete `DO NOT` / `MUST` directives convertible to lint rules |
...
LintGate v0.2 exposes 26 MCP tools. The recommended workflow is: **lint → drill-down → fix → verify**.
...
| Tool | Purpose |
|---|---|
| `extract_project_theory(path)` | Extract the full conceptual theory profile (6 facets) |
...
```bash
# Install LintGate + minimal required linter
pip install -e /path/to/lintgate
pip install ruff
...
```bash
# Minimal (just ruff — works out of the box)
pip install ruff

# Full suite
pip install ruff mypy radon bandit vulture

# MCP server
pip install mcp
```

LintGate auto-detects installed tools and enables them. No configuration needed.
...
│   ├── theory_extractor.py          # Project theory profiling (7 facets)
...
├── mcp_server.py                    # MCP tool interface (26 tools)
├── tests/                           # Test suites
