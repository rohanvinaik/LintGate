...
def test_compute_telemetry_summary_with_data():
    ...
    mock_entries = [
        {"event": "mcp_lint_run", "project": "/tmp/proj", "blocking_count": 1, "warning_count": 2, "info_count": 0, "files_count": 5, "duration_ms": 100},
        {"event": "mcp_lint_run", "project": "/tmp/proj", "blocking_count": 0, "warning_count": 1, "info_count": 1, "files_count": 3, "duration_ms": 50},
    ]

    with (
        patch("lintgate.telemetry._load_entries", return_value=mock_entries),
        patch("lintgate.telemetry.compute_token_economics_summary", return_value={"has_data": False})
    ):
        summary = compute_telemetry_summary("/tmp/proj")
