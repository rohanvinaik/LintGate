...
# ── Integration: fields appear in mutation results ───────────────


class TestTruthfulnessWarnings:
    def test_add_truthfulness_warnings_with_artifacts(self):
        from mcp_tools._mutation_tools_impl import _add_truthfulness_warnings

        results = [
            {"survival_interpretation": "DISCOVERY_ARTIFACT", "discovery_failed": True},
            {"survival_interpretation": "MEANINGFUL"},
        ]
        output: dict = {}
        _add_truthfulness_warnings(results, output)
        assert output["artifact_count"] == 1
        assert len(output["truthfulness_warnings"]) >= 1

    def test_add_truthfulness_warnings_with_mock_artifacts(self):
        from mcp_tools._mutation_tools_impl import _add_truthfulness_warnings
        ...
        _add_truthfulness_warnings(results, output)
        assert output["artifact_count"] == 1
        assert any("mock-boundary" in w for w in output["truthfulness_warnings"])

    def test_add_truthfulness_warnings_clean(self):
        from mcp_tools._mutation_tools_impl import _add_truthfulness_warnings

        results = [{"survival_interpretation": "MEANINGFUL"}]
        output: dict = {}
        _add_truthfulness_warnings(results, output)
        assert "truthfulness_warnings" not in output
        assert "artifact_count" not in output
