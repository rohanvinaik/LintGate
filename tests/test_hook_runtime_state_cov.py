...
class TestWriteDynamicRuntimeFiles:
    ...
    def test_success(self):
        with (
            ...
            reg.render_dynamic_for_targets.return_value = {
                ".claude/rules/runtime.md": "content"
            }
            success, status = write_dynamic_runtime_files("/tmp", mock.MagicMock())
        assert success
...
class TestRefreshRuntimeStateWithSession:
    def test_with_mesh_result(self):
        ...
        with (
            ...
            mock.patch(
                "lintgate.runtime_state.save_runtime_state_with_meta",
                return_value=save_result,
            ),
            mock.patch(
                "lintgate.write_scheduler.WriteScheduler.from_dict",
