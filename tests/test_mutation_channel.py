...
def test_mutation_channel_insufficient_depth(channel, tmp_path):
    ...
    assert result.severity == "warning" if "MUT001" in kinds and mock_state.survival_rate > 0.3 else "informational"
...
def test_mutation_channel_mutch007(channel, tmp_path):
    ...
    assert any("mutation_decompose" in block for str_list in mutch007_finding.suggestions for block in [str_list])
