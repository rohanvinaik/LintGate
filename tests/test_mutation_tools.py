...
def test_mutation_prescribe_tool(mock_engine, monkeypatch):
    ...
    mock_state.to_dict.return_value = {"func": "f", "survived_by_category": mock_state.survived_by_category}

    mock_engine.state_manager.state = {
        "/tmp/src/a.py::f": mock_state
    }
    ...
    assert "mutation_decompose" in result["next_actions"] or "mutation_refactor_loop" in result["next_actions"]
