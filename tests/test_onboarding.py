...
def test_ensure_project_venv_create_success(tmp_path: Path) -> None:
    ...
    with (
        ...
        patch("mcp_tools.onboarding_tools.subprocess.run", side_effect=[mock_create, mock_pip]),
    ):
        result = _ensure_project_venv(str(tmp_path))
...
def test_install_commands_with_uv(tmp_path: Path) -> None:
    with (
        patch("mcp_tools.onboarding_tools._project_venv_python", return_value="/v/bin/python"),
