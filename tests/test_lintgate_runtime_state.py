...
def test_runtime_state_to_dict_round_trip():
    ...
    original = RuntimeState(
        ...
        blocking_issues=0,
        warning_issues=0,
        symbol_coverage_blockers=0,
        coherence_state="test",
        prediction_accuracy=0.0,
        estimated_tokens_pct=0.0,
        compaction_count=0,
        tool_calls_total=0,
        top_constraint="test",
        approach_failures=0,
        prescriptive_spec_count=0,
        prescriptive_coverage_ratio=0.0,
    )
    ...
    assert reconstructed.symbol_coverage_blockers == original.symbol_coverage_blockers, (
        "symbol_coverage_blockers mismatch"
    )
    assert reconstructed.coherence_state == original.coherence_state, "coherence_state mismatch"
    assert reconstructed.prediction_accuracy == original.prediction_accuracy, (
        "prediction_accuracy mismatch"
    )
    assert reconstructed.estimated_tokens_pct == original.estimated_tokens_pct, (
        "estimated_tokens_pct mismatch"
    )
    ...
    assert reconstructed.approach_failures == original.approach_failures, (
        "approach_failures mismatch"
    )
    assert reconstructed.prescriptive_spec_count == original.prescriptive_spec_count, (
        "prescriptive_spec_count mismatch"
    )
    assert reconstructed.prescriptive_coverage_ratio == original.prescriptive_coverage_ratio, (
        "prescriptive_coverage_ratio mismatch"
    )
