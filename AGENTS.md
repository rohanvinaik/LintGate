...
> LintGate is a real-time quality supervision system with 74 MCP tools. It fires on
...
LintGate provides 74 MCP tools backed by 18 linters. Source of truth: `grep -Rho "@mcp.tool()" mcp_server.py mcp_tools/*.py | wc -l` (target `*.py` to avoid pycache matches).
...
Source of truth for tool count: `grep -Rho "@mcp.tool()" mcp_server.py mcp_tools/*.py | wc -l` (currently 74). Stale documentation has compounding negative effects — one wrong count propagates through every session that reads it.
