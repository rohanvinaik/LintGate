...
def persist_validation(project_root: str, gates: dict, ready: bool) -> str:
    """Persist validation result for apply to check."""
    out_path = Path(project_root) / _VALIDATION_FILE
    out_path.parent.mkdir(exist_ok=True)
    data = {"ready_to_apply": ready, "gates": gates}
...
def impl_rebuild_apply(
    ...
    if validation is None or not validation.get("ready_to_apply"):
        return str(
            helpers["_json_dumps"](
                {
