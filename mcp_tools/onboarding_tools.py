...
def _generate_pre_push_hook() -> str:
    ...
    lines = [
        ...
        'echo "[lintgate:gate:START:quality_infra]"',
        ...
        "else",
        '  echo "[lintgate:gate:FAIL:quality_infra]"',
        '  echo "[lintgate] quality infrastructure: INCOMPLETE — run setup_github_quality to fix"',
        "  exit 1",
        "fi",
        "",
        "# qlty analysis (mirrors CI qlty.yml)",
        'echo "[lintgate:gate:START:qlty]"',
        "if ! qlty check --all; then",
        '  echo "[lintgate:gate:FAIL:qlty]"',
        "  exit 1",
        "fi",
        "",
        "# Secrets scan (mirrors CI gitleaks check)",
        'echo "[lintgate:gate:START:gitleaks]"',
        'echo "[lintgate] scanning for secrets (gitleaks)"',
        "if ! gitleaks detect --source . --no-banner --redact; then",
        '  echo "[lintgate:gate:FAIL:gitleaks]"',
        '  echo "[lintgate] BLOCKED: secrets detected — fix before pushing."',
        "  exit 1",
        "fi",
        "",
        'echo "[lintgate:gate:START:fast_symbol_precheck]"',
        'if [ "${LINTGATE_SKIP_FAST_SYMBOL_PRECHECK:-0}" = "1" ]; then',
        '  echo "[lintgate] fast symbol precheck skipped (LINTGATE_SKIP_FAST_SYMBOL_PRECHECK=1)"',
        "else",
        '  if ! python scripts/fast_symbol_precheck.py --project-root "$REPO_ROOT"; then',
        '    echo "[lintgate:gate:FAIL:fast_symbol_precheck]"',
        "    exit 1",
        "  fi",
        "fi",
        "",
        ...
        "    BASE=''",
        "  fi",
        '  echo "[lintgate:gate:START:symbol_gate]"',
        '  if [ -n "$BASE" ]; then',
        "    if ! python -m lintgate.symbol_gate_runner \\",
        '      --project-root "$REPO_ROOT" \\',
        "      --coverage-json coverage.json \\",
        '      --base "$BASE" --head HEAD --surface ci; then',
        '      echo "[lintgate:gate:FAIL:symbol_gate]"',
        "      exit 1",
        "    fi",
        "  else",
        "    if ! python -m lintgate.symbol_gate_runner \\",
        ...
        '    echo "[lintgate:gate:START:pip_audit]"',
        '    echo "[lintgate] running pip-audit (supply-chain scan)"',
        "    if ! pip-audit; then",
        '      echo "[lintgate:gate:FAIL:pip_audit]"',
        "      exit 1",
        "    fi",
        "  fi",
        "fi",
        ...
        '  echo "[lintgate:gate:START:sonar]"',
