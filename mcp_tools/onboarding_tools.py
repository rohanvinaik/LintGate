...
def _generate_sonar_properties(github: dict[str, Any], layout: dict[str, Any]) -> str:
    ...
    exclude_parts = layout.get("exclude_patterns", [])
    exclusions = ",\\\n  ".join(f"{p}**" if not p.endswith("**") and not p.endswith("**/*") else p
...
def _generate_badge_markdown(github: dict[str, Any], layout: dict[str, Any]) -> str:
    ...
    badges: list[str] = [
        f"[![Maintainability](https://api.codeclimate.com/v1/badges/PLACEHOLDER/maintainability)]"
...
def _inject_badges_into_readme(
    ...
    for fingerprint in _BADGE_FINGERPRINTS:
...
def register(mcp, helpers):
    ...
    def getting_started(
        ...
        # Suggest GitHub quality setup when remote detected but configs missing
    ...
    def setup_github_quality(
        ...
        qlty_result: dict[str, Any] = {
            ...
            "note": ".qlty is intentionally gitignored for this repository.",
        }
        ...
        if qlty_exists:
            ...
            # Also create .qlty/.gitignore to keep cache out of repo
        ...
        # Note: .qlty/ is gitignored — not staged
