...
def register(mcp: Any, helpers: Any) -> dict[str, Any]:
    ...
    def test_characterize_mark(
        ...
        """
        project_root = helpers["_validate_project_root"](path)
        return _impl_test_characterize_mark(project_root, test_file, maturity)
...
def _generate_characterization_test(
    ...
    for p in params:
        ...
        if "str" in hint:
            param_lines.append(f'    {p["name"]} = ""  # TODO: fill from call site')
        elif "int" in hint:
            param_lines.append(f"    {p['name']} = 0  # TODO: fill from call site")
        elif "bool" in hint:
            param_lines.append(f"    {p['name']} = False  # TODO: fill from call site")
        elif "list" in hint or "List" in hint:
            param_lines.append(f"    {p['name']} = []  # TODO: fill from call site")
        elif "dict" in hint or "Dict" in hint:
            param_lines.append(f"    {p['name']} = {{}}  # TODO: fill from call site")
        else:
            param_lines.append(f"    {p['name']} = None  # TODO: fill from call site")
